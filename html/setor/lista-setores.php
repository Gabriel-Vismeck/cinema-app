...
$sql = "SELECT
            Setor.*,
            COUNT(Funcionario.idFuncionario) AS 'numeroFuncionarios'
        FROM Setor
        LEFT JOIN Funcionario
            ON Setor.idSetor = Funcionario.idSetor
...
<head>
    ...
    <title>Cinemáximo - Lista de Setores</title>
    ...
    <link rel="stylesheet" href="/Projeto-Cinema/css/telas/lista-setores.css">
...
<body>
    <div class="conteudo">
        ...
        <h1 class="text-center my-4" style="color: var(--branco);">Lista de Setores</h1>

        <a class="botao-componente" href="cadastro-setor.php">Cadastrar setor</a>

        <div class="lista fundo-cinza my-4">
            <div class="linha linha-cabeca">
                <div>Nome</div>
                <div>Número de Funcionários</div>
                <div></div>
            </div>
            <?php
            foreach ($setores as $linha) {
                echo "
                        <div class='linha'>
                            <div>{$linha["nome"]}</div>
                            <div>{$linha["numeroFuncionarios"]}</div>
                            <div class='item-acoes'>
                                <a href='cadastro-setor.php?setor={$linha["idSetor"]}'>
                                    <i class='item-animacao fa-solid fa-pen'></i>
                                </a>
                                <a href='excluir-setor.php?setor={$linha["idSetor"]}'>
